...
//        return $this->render($element);
    ...
    public function render(ElementInterface $element){
        if(!$element instanceof SelectElement){
            throw new Exception\InvalidArgumentException(sprintf('%s requires that the element is of type Zend\Form\Element\Select',
                __METHOD__));
        }

        $name = $element->getName();
        if(empty($name) && $name !== 0){
            throw new Exception\DomainException(sprintf('%s requires that the element has an assigned name; none discovered',
                __METHOD__));
        }
        ...
        $rendered = sprintf('<select %s>%s</select>',
    ...
    public function renderOptions(array $options, array $selectedOptions = []){
        ...
        foreach($options as $key => $optionSpec){
            ...
            if(isset($optionSpec['options']) &&
                ...
                $optionStrings[] =
                    $this->renderOptgroup($optionSpec, $selectedOptions);
                continue;
            }
            ...
            if(isset($optionSpec['attributes']) &&
                ...
                $attributes =
                    array_merge($attributes, $optionSpec['attributes']);
            }

            $this->validTagAttributes = $this->validOptionAttributes;
            $optionStrings[] =
                sprintf($template, $this->createAttributesString($attributes),
                    $escapeHtml($label));
        }
    ...
    protected function validateMultiValue($value, array $attributes){
        ...
        if(!isset($attributes['multiple']) || !$attributes['multiple']){
            throw new Exception\DomainException(sprintf('%s does not allow specifying multiple selected values when the element does not have a multiple attribute set to a boolean true',
                __CLASS__));
        }
